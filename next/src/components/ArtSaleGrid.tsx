...
export default function ArtSaleGrid({
    ...
    if (featuredArtworks.length > 0) {
        ...
        <section className="py-16 px-4 max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-[repeat(auto-fit,_minmax(30%,_1fr))] gap-6">
...
">
  <div className="bg-[#f0ead8] p-6 sm:p-5 md:p-5 lg:p-5">
                ...
                <small>{artwork.workType.Tittel} - {artwork.category.Tittel}</small><br />
                {artwork.price ? <small className="italic">Kr. {artwork.price}</small> : <small className="italic"> Ta kontakt for pris</small>}
                </div>
              </Link>
            ))}
          </div>
